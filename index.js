...
app.post("/write-to-sheet", async (req, res) => {
  ...
  const auth = new google.auth.GoogleAuth({
    keyFile: "./googlesheet_chimali.json", // Ruta al archivo JSON de credenciales
    scopes: "https://www.googleapis.com/auth/spreadsheets",
  });

  const client = await auth.getClient();
  const googleSheets = google.sheets({ version: "v4", auth: client });
  const spreadsheetId = "1XR7pIQNz5pXaY2j3w7nj0w5M9kLtLfA6A-tm0YtXGXQ"; // El ID de tu hoja de Google Sheets
